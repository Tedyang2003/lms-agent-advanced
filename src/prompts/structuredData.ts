export const STRUCTURED_DATA_QUERY_TOOL_DESCRIPTION =
    `Look up data, numbers, rows, or aggregates from an uploaded spreadsheet, CSV, or JSON data ` +
    `file by exact fileName. If the user references something ambiguous ("that model," "it"), ` +
    `resolve it from conversation history first — ask the user if still unclear, never guess or ` +
    `default to "all". This tool's question is sent to a sub-agent with no view of this ` +
    `conversation: write it fully self-contained, including any relevant criteria or context, not ` +
    `implicit references.`;

export const LIST_TABLES_DESCRIPTION =
...
export const QUERY_TABLE_DESCRIPTION =
    `Runs a read-only SQL query against the loaded tables. ` +
...
export function buildStructuredDataSubAgentSystemPrompt(schemaSummary: string): string {
    return (
        `You are a structured-data query agent (handles spreadsheets, CSV, and JSON files alike, ` +
        `all queried the same way via SQL). Schema for the relevant file(s) has already been ` +
        `fetched:\n${schemaSummary}\n\n` +
        `CRITICAL: You must call query_table to answer ANY question, as an actual tool ` +
        ...
        `answered by calling query_table with arguments {"sql": "SELECT AVG(score) AS avg_score ` +
        ...
        `Use query_table directly with this schema — you should rarely need to call ` +
        `list_tables again. Write correct SQL using the exact column names shown above. ` +
        ...
        `original row number in the source file (not shown in the schema above, but it's real — SELECT it ` +
...
export const STRUCTURED_DATA_SUBAGENT_RETRY_LOG_MESSAGE =
...
export const STRUCTURED_DATA_SUBAGENT_RETRY_USER_MESSAGE =
    `You did not get a usable answer from query_table — you either wrote SQL as ` +
